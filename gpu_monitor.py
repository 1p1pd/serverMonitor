...
def get_gpu_infos(nvidiasmi_output):
    ...
    for idx, gpu in enumerate(gpus):
        ...
        gpu_infos.append({'idx': idx, 'model': model, 'pids': pids, 'used_mem': used_memory, 'total_mem': total_memory})
...
def print_gpu_infos(server, gpu_infos, run_ps, run_get_real_names,
    ...
    for gpu_info in gpu_infos:
        ...
        if len(gpu_info['pids']) == 0:
            ...
            status = '{} out of {} used by {}'.format(used_mem, total_mem, ', '.join(users))
...
def getServerInfo(argv):
    ...
    for server in args.servers:
        ...
        server_infos[server] = print_gpu_infos(server, gpu_infos, run_ps, run_get_real_names,
